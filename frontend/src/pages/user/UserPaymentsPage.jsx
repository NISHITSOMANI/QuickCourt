...
const UserPaymentsPage = () => {
  ...
  const { data: paymentsData, isLoading, error } = useQuery(
    ['user-payments', currentPage, statusFilter, dateFilter, searchQuery],
    async () => {
      try {
                    ...
                    {paymentsData.payments.map((payment) => (
                      <tr key={payment.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            <div className="text-sm font-medium text-gray-900">{payment.booking?.venue?.name || payment.venue}</div>
                            <div className="text-sm text-gray-500">
                              {payment.booking?.court?.sport || payment.sport} • {payment.booking?.court?.name || payment.court}
                            </div>
                            <div className="text-sm text-gray-500">
                              {new Date(payment.booking?.startTime || payment.date).toLocaleDateString()} • 
                              {new Date(payment.booking?.startTime || payment.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - 
                              {new Date(payment.booking?.endTime || payment.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-600">
                {searchQuery || statusFilter || dateFilter 
                  ...
                  : "You haven't made any payments yet. Start booking venues to see your payment history!"
                }
              </p>
            </div>
          )}
